<template>
  <div id="app">
    <div class="todo-container">
      <div class="todo-wrap">
        <TodoHeader :receiveData="receiveData" />
        <TodoList
          :todoList="todoData"
          :checkTodo="checkTodo"
          :deleteTodo="deleteTodo"
        />
        <TodoFooter
          :todoList="todoData"
          :checkTodo="checkTodo"
          :checkAllTodo="checkAllTodo"
          :deleteAllData="deleteAllData"
        />
      </div>
    </div>
  </div>
</template>

<script>
import TodoHeader from "./components/TodoHeader.vue";
import TodoFooter from "./components/TodoFooter.vue";
import TodoList from "./components/TodoList.vue";
export default {
  name: "App",
  components: { TodoHeader, TodoFooter, TodoList },
  data() {
    return {
      todoData: JSON.parse(localStorage.getItem("todoData")) || [],
    };
  },
  methods: {
    // 给header传值
    receiveData(message) {
      console.log("receive header message", message);
      this.todoData.unshift(message);
    },
    // 给footer传值
    receiveMessage(footerMessage) {
      console.log("receive footer message", footerMessage);
    },
    // 勾选或者取消勾选
    checkTodo(id) {
      console.log(id);
      this.todoData.forEach((todo) => {
        if (todo.id === id) {
          todo.done = !todo.done;
        }
      });
    },
    deleteTodo(id) {
      this.todoData = this.todoData.filter((todo) => {
        return todo.id !== id;
      });
    },
    // 全选
    checkAllTodo(done) {
      this.todoData.forEach((todo) => {
        todo.done = done;
      });
    },
    // 全部删除
    deleteAllData() {
      // filter不影响原数组，所以要重新赋值
      this.todoData = this.todoData.filter((todo) => {
        return !todo.done;
      });
    },
  },
  watch: {
    todoData: {
      deep: true,
      handler(value) {
        localStorage.setItem("todoData", JSON.stringify(value));
      },
    },
  },
};
</script>

<style>
/*base*/
body {
  background: #fff;
}
.btn {
  display: inline-block;
  padding: 4px 12px;
  margin-bottom: 0;
  font-size: 14px;
  line-height: 20px;
  text-align: center;
  vertical-align: middle;
  cursor: pointer;
  box-shadow: inset 0 1px 0 rgba(255, 255, 255, 0.2),
    0 1px 2px rgba(0, 0, 0, 0.05);
  border-radius: 4px;
}
.btn-danger {
  color: #fff;
  background-color: #da4f49;
  border: 1px solid #bd362f;
}
.btn-danger:hover {
  color: #fff;
  background-color: #bd362f;
}
.btn:focus {
  outline: none;
}
.todo-container {
  width: 600px;
  margin: 0 auto;
}
.todo-container .todo-wrap {
  padding: 10px;
  border: 1px solid #ddd;
  border-radius: 5px;
}
/*header*/
/*main*/
/*item*/
/*footer*/
</style>
